...
function Index() {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#070605] text-[#f8f2e7]">
    ...
    <main id="inicio">
      ...
      {/* Como funciona — clara */}
      <section id="como" className="bg-[#eee5d7] px-5 py-16 text-[#19140e] lg:px-8"><div className="mx-auto max-w-7xl text-center"><p className="text-xs font-bold uppercase tracking-[.3em] text-[#8B6914]">Como funciona</p><h2 className="mt-4 font-serif text-4xl sm:text-5xl">Um caminho simples para começar</h2><div className="mt-8 grid gap-4 md:grid-cols-4">{[["01","Converse conosco","Clique no WhatsApp e conte brevemente o que você procura."],["02","Entenda a necessidade","O primeiro contato ajuda a orientar o próximo passo."],["03","Construa seu cuidado","O acompanhamento considera sua realidade e suas necessidades."],["04","Avance com segurança","Você segue com orientação e acompanhamento profissional."]].map(([n,t,d])=><div key={n} className="rounded-[1.8rem] border border-[#8B6914]/15 bg-white/70 p-6 text-center shadow-[0_12px_35px_rgba(80,55,10,.06)]"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[#C9A227]/15 font-serif text-lg text-[#8B6914]">{n}</span><h3 className="mt-4 font-serif text-2xl">{t}</h3><p className="mt-2 text-sm leading-relaxed text-[#19140e]/60">{d}</p></div>)}</div><a href={wa} target="_blank" rel="noreferrer" className={btn + " mt-8"}>Falar pelo WhatsApp <MessageCircle className="h-5 w-5"/></a></div></section>
